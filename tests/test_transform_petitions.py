...
def test_count_word_occurrences(spark_session):
    df = spark_session.createDataFrame([Row(word="should"), Row(word="attend"), Row(word="should")])
    word_counts = count_word_occurrences(df)
    assert word_counts.filter(col("word") == "should").select("count").collect()[0][0] == 2

def test_get_common_words(spark_session):
    df = spark_session.createDataFrame([Row(word="should"), Row(word="attend")])
    common_words = get_common_words(df, limit=1)
    assert common_words == ["should"]
